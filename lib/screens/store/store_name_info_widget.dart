...
import 'package:alcoholic/models/competitions/selection_clock.dart';
import 'package:alcoholic/screens/competition/competition_result_widget.dart';
...
import '../../models/competitions/read_only_clock.dart';
...
class StoreNameInfoWidgetState extends State<StoreNameInfoWidget> {
  ...
  late ReadOnlyClock readOnlyClock;
  ...
  void initState() {
    ...
    groupsStream =
        competitionController.readGroups(widget.storeNameInfo.sectionName);
  }
  ...
  StreamBuilder<DocumentSnapshot> readFromReadOnlyClock(
    String coundDownClockId,
    StoreDraw latestStoreDraw,
  ) {
    return StreamBuilder<DocumentSnapshot>(
        stream: competitionController.retrieveReadOnlyClock(coundDownClockId),
        builder: ((context, snapshot) {
          if (snapshot.hasData && snapshot.data!.exists) {
            readOnlyClock = ReadOnlyClock.fromJson(snapshot.data);
            ...
            if (readOnlyClock.remainingTime < 0) {
              // Show remaining time before competition starts.
              return WaitWidget(
                storeDraw: latestStoreDraw,
                remainingDuration:
                    Duration(seconds: readOnlyClock.remainingTime * -1),
                pickWonPrice: false,
                showPlayIcon: false,
                showAlarm: false,
            ...
            // Show grand prices with play icon.
                ...
                readOnlyClock.remainingTime < competitionTotalDuration) {
              return WaitWidget(
                remainingDuration:
                    Duration(seconds: readOnlyClock.remainingTime),
                storeDraw: latestStoreDraw,
                pickWonPrice: false,
                showPlayIcon: true,
                showAlarm: false,
            ...
            // Show and pick grand prices.
                ...
                readOnlyClock.remainingTime < grandPricePickingDuration) {
              return WaitWidget(
                remainingDuration:
                    Duration(seconds: readOnlyClock.remainingTime),
                storeDraw: latestStoreDraw,
            ...
            // Show group picking
                ...
                readOnlyClock.remainingTime <= groupPickingDuration) {
            ...
            else if (readOnlyClock.remainingTime <= competitionTotalDuration) {
